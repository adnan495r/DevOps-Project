...
echo "Verifying that mysql_db database exists..."
if mysql -h $CONN_HOST -P $CONN_PORT -u $MYSQL_USERNAME -p$MYSQL_PASSWORD -e "USE mysql_db;" > /dev/null 2>&1; then
  echo "Database mysql_db exists and is accessible"
else
  echo "WARNING: Could not access mysql_db database! Creating it now..."
  mysql -h $CONN_HOST -P $CONN_PORT -u $MYSQL_USERNAME -p$MYSQL_PASSWORD -e "CREATE DATABASE IF NOT EXISTS mysql_db;"
fi
...
echo "Login with username / db password"
exit 0