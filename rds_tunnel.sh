...
if ! which mysql > /dev/null 2>&1; then
  ...
  if ! which mysql > /dev/null 2>&1; then
    ...
    echo "MySQL client installed successfully using dnf package manager."
  fi
else
...
mysql -h localhost -P 3307 -u ${mysql_username} -p${mysql_password} mysql_db
EOF
...
server {
    listen 80;
    server_name aali-bi.devops42.online;
...
if sudo docker ps -a | grep -q metabase; then
  ...
  sudo docker run -d -p 3000:3000 --name metabase \
    ...
    -e "MB_SETUP_ADMIN_EMAIL=adnan@example.com" \
    -e "MB_SETUP_ADMIN_FIRST_NAME=Admin" \
    -e "MB_SETUP_ADMIN_LAST_NAME=User" \
    -e "MB_SETUP_ADMIN_PASSWORD=Question42!" \
